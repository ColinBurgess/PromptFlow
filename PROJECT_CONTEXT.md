...
-   **Web Client ID (for Google Sign-In):** `***REMOVED***` (Should be stored securely, e.g., in `strings.xml` or build config, not hardcoded directly if possible, or fetched from `google-services.json` processing).
-   **SHA-1 Debug Fingerprint:** `***REMOVED***` (Ensure this is added to the Firebase project settings for debug builds).
